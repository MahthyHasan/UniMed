...
function CommonLogin() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            localStorage.setItem("user_Id", patient.id);
            ...
            // Check bioalert field in UserBio
            const bioResponse = await axios.get(
                `http://localhost:8088/api/v1/user/bio/${patient.id}`
            );
            const userBio = bioResponse.data;

            // Automatically dismiss the alert after 3 seconds and navigate to appropriate page
            setTimeout(() => {
                setAlertVisible(false);
                if (userBio.bioalert === false) {
                    navigate("/BioFill");
                } else {
