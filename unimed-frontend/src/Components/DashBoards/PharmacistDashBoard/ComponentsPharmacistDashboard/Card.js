...
const Card = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const checkedInResponse = await axios.get(
          `http://localhost:8088/api/v1/booking/countByStatus?status=checked_in`
        );
        const checkedOutResponse = await axios.get(
          `http://localhost:8088/api/v1/booking/countByStatus?status=consulted`
        );
